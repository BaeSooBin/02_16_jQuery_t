;(function($){
    ...
    var intro = {
        init:function(){
            var that = this; //this를 변수에 사용하기 좋음
                that.navFn();
                that.mainSlideFn();
                that.noticeFn();
                that.galleryFn();
        ...
        navFn:function(){
            //아코디언 메뉴
            //메인메뉴 nav (선택자(selector): .main-btn)에 버튼 이벤트 리스너 마우스
            //오버 시(mouseenter) 해당 메뉴에 클래스 추가(addClass('on'))하여 스타일을 설정하고 
            ...
            var $navUl = $('#nav > ul'); //이 영역을 떠나면 모든 메뉴 초기화
                ...
                $mainBtn.on({
                    mouseenter:function(){
                        $mainBtn.removeClass('on'); //버튼 모든 추가된 클래스('on') 삭제
                        $(this).addClass('on');
                        $sub.removeClass('on');
        ...
        mainSlideFn:function(){
            ...
            var cnt = 0;
                ...
                $nextBtn.on({
                    click:function(){
                        //연속해서 클릭하면 버블링 발생
                        //디버깅 : 애니메이션이 동장이 안될때 클릭하도록 제어문 설정(if 조건문)
                        //not(부정문) => != !==
                        //애니메이션이 안될때를 부정문(!) !$slideWrap.is(':animate')
